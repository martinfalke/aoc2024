defmodule Utils do
  ...
  def read_input_file(day, :example) do
    filepath = day < 10 && "lib/inputs/example/day0#{day}.txt" || "lib/inputs/example/day#{day}.txt"
    {:ok, f} = File.read(filepath)
    f
  end
  ...
  def remove_trailing_newline([head | tail]) when tail != [], do: [head | remove_trailing_newline(tail)]
